...
(defmacro with-buffer-dispatch (buffer &body body)
  (error "with-buffer-dispatch shouldn't be used in LispWorks: ~S."
...
(declaim (inline ascii-stream-end-of-file-p))

(defun ascii-stream-end-of-file-p (stream)
  (declare #.*optimization*)
  (char= (the base-char (ascii-stream-peek-char stream #\EOT)) #\EOT))
