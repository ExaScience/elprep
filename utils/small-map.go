...
It does so by either setting the value of the first entry that has the
same key as the given key, or else by appending a new key/value pair
to the end of the SmallMap if no entry already has that key.
*/
...
/*
Delete returns a SmallMap from which the first entry has been removed
that has the same key as the given key.
