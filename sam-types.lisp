...
(defun sam-alignment-buffer-emptyp (buf)
  (declare (sam-alignment-buffer buf) #.*optimization*)
  (buffer-emptyp (sam-alignment-buffer-line buf)))
...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (defun sam-alignment-buffer-accessor-name (symbol)
    (let* ((sname (symbol-name symbol))
           (subsname (subseq sname 1 (1- (length sname)))))
      (intern (concatenate 'simple-base-string "SAM-ALIGNMENT-BUFFER-" subsname) :elprep))))
...
(define-sam-alignment-buffer-accessor         +rname+)
(define-sam-alignment-buffer-integer-accessor +pos+)
(define-sam-alignment-buffer-integer-accessor +mapq+)
(define-sam-alignment-buffer-accessor         +cigar+)
(define-sam-alignment-buffer-accessor         +rnext+)
(define-sam-alignment-buffer-integer-accessor +pnext+)
(define-sam-alignment-buffer-integer-accessor +tlen+)
(define-sam-alignment-buffer-accessor         +seq+)
(define-sam-alignment-buffer-accessor         +qual+)
