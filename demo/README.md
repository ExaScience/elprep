...
* [run-wgs-gatk.sh]: a bash script that runs the 5-step preparation pipeline using elPrep split/filter/merge on NA12878 whole-genome data. 
...
The input data for demo 4 is 114 GB large and can be downloaded manually [from ebi directly](http://www.ebi.ac.uk/ena/data/view/ERS189474).  
...
## Demo 4: a preparation pipeline using elprep/split/filter/merge on whole-genome data

The script run-wgs-gatk.sh executes a 5-step pipeline that consists of removing the unmapped reads, replacing the reference sequence dictionary, adding read groups, marking duplicates, and sorting by coordinate order. The script executes this pipeline using the elprep split/filter/merge approach. This script is only executable for the whole-genome data (NA12878_S1.bam). Demo 3 provides a similar script for the whole-exome data.

1) By default, the scripts use the maximum number of available threads, based on your processor's capabilities. If you want to use a different number of threads, edit the scripts to do so (cf. 2nd line).

2) By default, the script stores intermediate split data in the .bam format, see the --intermediate-files-output-type option. If you want to change to .sam or .cram, please edit this option.
