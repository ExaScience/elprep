#!/bin/bash
input=NA12878.exome.hg19 #without extension
ref=hg19
...
then
	    elprep sfm $input.bam $input.sfm.out.bam --mark-duplicates --mark-optical-duplicates $input.sfm.out.metrics --sorting-order coordinate --bqsr $input.sfm.out.recal --known-sites $sites1.elsites,$sites2.elsites --bqsr-reference $ref.elfasta --filter-non-overlapping-reads $bed.bed
else
