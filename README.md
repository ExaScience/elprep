...
This filter is used for replacing the header of a .sam/.bam/.cram file by a given header. The given header is passed as a single argument following the command option. The format of the header can either be a .dict file or another .sam/.bam/.cram file from which to extract the header.

In contrast to the --replace-reference-sequence-file option, the new header is used verbatim. There are no checks on preserving the sorting order, there is no filtering of reads that map to entries that are no longer present in the new header, and so on.

The use of this option is required when processing headerless files created by the elprep split tool. The elprep split commands splits up a .sam/.bam/.cram file into multiple headerless files and a separate header file. This header file needs to be passed via the --header option for processing the headerless split files. For more details see the elprep split command.  
...
The split command outputs three types of files: 

1. a subdirectory "/path/to/output/splits/" containing a file per entry in the sequence dictionary of the input file that contains all reads that map to that entry. These files do not contain a header. 
2. a "/path/to/output/output-prefix-header.dict" file containing a header that contains meta information to be used by elPrep for processing the files in the "/splits/" directory.
3. a "/path/to/output/output-prefix-spread.output-type" file containing all reads of which the mate maps to a different entry in the sequence dictionary of the input file. This file has the same header as the input file.

To process the files created by the elprep split command, one needs to call the elprep command for each entry in the path/to/output/splits/ directory as well as the /path/to/output/output-prefix-spread.output-type file. To process the files in the "split" directory one needs to pass the header generated by the split command (/path/to/output/output-prefix-header.dict) via the --header option. To take advantage of parallel sorting, the option --split-file should also be passed to the elprep command. No special options are necessary for processing the reads in the spread reads file (/path/to/output/output-prefix-spread.output-type). Finally, the output files produced this way, need to be merged with the elprep merge command. For example scripts see "elprep-sfm.py" and "elprep-sfm-gnupar.py".
