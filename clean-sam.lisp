...
(hcl:defglobal-variable *cigar-consumes-reference-bases-table*
...
(defun sam-alignment-end (aln cigar-list)
  ...
  (let ((cigar-reference-length
         (loop with table = *cigar-consumes-reference-bases-table*
               for (key . val) of-type (symbol . int32) in cigar-list
               when (svref table (- (char-code (sbchar (symbol-name key) 0)) +min-cigar-operation+)) 
...
(defun operator-consumes-read-bases-p (operator)
  ...
  (or (eq operator :M) (eq operator :I) (eq operator :S) (eq operator :=) (eq operator :X)))
...
(defun operator-consumes-reference-bases-p (operator)
  ...
  (or (eq operator :M) (eq operator :D) (eq operator :N) (eq operator :=) (eq operator :X)))
...
(defun get-read-length-from-cigar (cigar-list)
  ...
  (declare (list cigar-list) #.*optimization*)
    (loop for (operator . val) of-type (symbol . int32) in cigar-list
...
(defun element-stradless-clipped-read (new-cigar operator relative-clipping-position clipped-bases)
  (declare (stream new-cigar) (symbol operator) (int32 relative-clipping-position clipped-bases) #.*optimization*)
  (if (operator-consumes-read-bases-p operator)
      (if (operator-consumes-reference-bases-p operator)
          (when (> relative-clipping-position 0)
            (format new-cigar "~a~d" operator relative-clipping-position))
...
(defun soft-clip-end-of-read (clip-from cigar-list)
  ...
  (let ((pos 0))
    (declare (int32 pos))
    (decf clip-from)
    (with-output-to-string (new-cigar)
      (loop named cigar-loop for (operator . val) of-type (symbol . int32) in cigar-list
            do (let ((end-pos (+ pos (if (operator-consumes-read-bases-p operator) val 0))))
                 (declare (int32 end-pos))
                 (if (< end-pos clip-from)
                     (format new-cigar "~a~d" operator val)
