                      ...
                      (lambda ()
                        ...
                        (if (= (length table) 1) ; only one bucket, so no bucket distribution necessary (for example, for unmapped split files)
                          ...
                          (macrolet ((receive-chunks (index-form) ; distribute over buckets according to index-form
                            ...
                            (if split-file
                              (let ((mm (sam-header-user-tag header :|@mm|)))
                                (assert (null (cdr mm)))
                                (setq mm (car mm))
                                (if mm
                                  (let* ((min-pos (parse-int32 (getf mm :|mi|) 1))
                                         (max-pos (parse-int32 (getf mm :|ma|) #.(1- (expt 2 31))))
                                         (bucket-size (/ (- max-pos min-pos) threads)))
                                    (declare (int32 min-pos max-pos) (rational bucket-size))
